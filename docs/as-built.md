...
| `pre-compact.js` | PreCompact | 기록 꼬리 → `claude -p` 요약 → `handoff-compact.md`, 방에 "정리 중" (알림 계정) | fail-open. 못 썼으면 "못 썼다"를 파일에 적는다 |
...
| `PRODEV_NOTIFY_ROOM` | pre-compact.js | 봉투의 `chat_id` 가 먼저. 둘 다 없으면 알림 건너뜀 |
