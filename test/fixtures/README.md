...
**2026-09-15 ADR-039 (방 하나)**: 확정 조건 ② 가 "같은 과제의 방" 이 되어 fixture.db 는 그대로 쓴다 — 방 2(`/files`) 는 이관된 옛 방으로 읽힌다. E-0006 은 이제 조건 ② 를 통과하고 ④(직전 봇 글에 카드 번호 없음)에서 막힌다 — 사례 이름을 그렇게 고쳤다. 조건 ② 를 어기는 판(다른 과제의 방)은 `hooks.test.js` 의 ADR-039 시험 둘이 fixture.db 사본에서 글을 옮겨 잰다.

## 토큰은 지운 것이다 (2026-09-11)

저장소가 public 이 되어 살핀 결과, `chat/minidiscord.db` 에 **살아 있는 봇 토큰 다섯과 로그인 세션 서른**이 그대로 있었다.
서버가 127.0.0.1 에만 열려 실해는 없었으나 공개 저장소에 둘 값이 아니다.

**지금 두 fixture DB 는 토큰을 지운 것이다** (`../meta/prodev-review/scripts/tools/db-scrub.js`) —
...
**다시 만들 때도 지운 판으로 준다.** 살아 있는 서버 DB 를 그대로 복사하지 않는다.
