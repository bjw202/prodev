// find.js 시험 — fixtures/find/questions.json 의 물음 10 + 조사 변형 3 + 없는 것 1 = 14건.
// 물음 하나가 test() 하나다. 기대(층·경로)는 meta 가 준 questions.json 에만 있고 여기 베껴 적지 않는다.
//
// 재는 법 (ADR-016):
//   1~4층 — 층이 정확히 같고, 답이 기대 경로 하나여야 한다.
//   5층(대화) — 층이 정확히 같고, 기대 #id 가 결과 안에 있으면 된다 (여러 건이 나오는 층이다).
//   없는 것 — 층이 null 이고 결과가 비어야 한다.
//
// 4층(대화)은 fixtures/chat/minidiscord.db 를 MINIDISCORD_DB 로 가리켜 chat.js 를 부른다.
...
// 과제 폴더를 임시로 짓고 색인을 한 번 돌린 뒤, 14건을 한 번씩 물어 답을 모은다.
function askAll() {
