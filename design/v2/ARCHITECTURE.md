...
**`## 결과` 절 규격 (ADR-028)** — 갈래별 요약 표와 원본 실마리를 **둘 다** 싣는다. 하나만 싣지 않는다.
...
규격 초과 5건: 3행 · 11행 · 17행 · 22행 · 30행 (files.md 의 "이상한 자리")
전체 표: inbox/20260909-part-incoming/inspection.csv (열 A~F · 34행) · files.md
```

- **갈래별 요약 표**는 `conditions` 의 가르는 축마다 하나씩 — 장비별 · 공급사별 · 판정별 수.
  물음은 언제나 "무엇별로" 들어온다 ("BSTech 것만" · "CH-2B 는" · "규격 이내인 것은 몇 개였지").
- **원본 실마리**는 한 줄 — 전체 표가 어느 파일의 몇 행인지. 정상 행 서른을 카드에 베끼지 않는다.
- 이상 자리만 실은 카드는 이상한 것만 답한다. 그러면 봇이 원본 csv 를 직접 읽고, 카드가 일을 안 한 것이다.
