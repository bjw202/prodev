...
| **prodev** (이 저장소, 제작 세션) | 스크립트·훅·설정·setup 을 만든다 · `/harness:harness` 를 돌려 스킬·에이전트를 만든다 · 단위 시험을 쓴다 · 봇을 띄운다 · **단계 끝마다 `docs/as-built.md` 와 `docs/log.md` 를 갱신한다** | 예측을 정하지 않는다 · 자기 검수표를 스스로 채우지 않는다 · 검수 자료의 정답지를 읽지 않는다 |
...
- 할 일: 머리말 파서(YAML 부분집합: 스칼라·리스트·`{k: v}`) → `index.md`·`index.json` (`errors[]`, `next E`) · find.js **여섯 층**(ADR-026 으로 과제 문서 층이 늘었다) + 조사 떼기 + 접기 + void 따라가기 + `find.log`(칸 여섯)
- 끝 조건: `test/index.test.js` — fixture 카드 12(정상 10 · 깨진 머리말 1 · void 1): 표 행 수 = 11, errors = 1, exit 1. `test/find.test.js` — 물음 **18 칸** → 기대 층·경로 전부 일치, 그 가운데 `예산 감광액` 은 **안 걸려야** 한다 (fixture 는 meta 가 준다, T1.M)
- 크기: 1일
...
### T1.M 1단계 관문 — meta 지시
- prodev: `docs/as-built.md` · `docs/log.md` 갱신 뒤 관문을 청한다 (문맥이 압축되면 사라진다. 파일이 진실이다)
...
## 5단계 — 진화하는 비서 (v3 · ADR-031)

**쓸수록 이 사람에게 맞아 가는 비서를 만든다.** 관문 셋으로 나누고 **하나씩** 간다 — 앞 관문이 판정을 받기 전에 다음 것을 시작하지 않는다. 근거 문서는 `../meta/prodev-review/plans/2026-09-11-handoff-evolution.md`.

4단계(실전 2주)와 순서가 얽히지 않는다. 4단계는 회사에서 하고 이 단계는 **이 맥에서 굳힌다** — 회사에는 관문도 예측도 정답지도 없기 때문이다 (ADR-031).

### T5.A 관문 A — 자리 (폴더 · 훅 · 설정) — meta 지시
- 굳은 것이 **살 자리**를 만든다. 판단이 없는 배관이다: `analysis/` · `templates/` · `house.md` · `.gitignore` · 훅의 여덟째 절 · 허용 목록 축소 · Git Bash 길 (ADR-032 · 033)
- 끝 조건: 단위 시험이 넷을 보고, `npm test` · `npm run test:server` 실패 0. meta 가 직접 돌려 센다

### T5.B 관문 B — 길 (스킬 · 규칙) — meta 지시
- **굳는 길** (ADR-035): 사람이 "앞으로" 라 하면 굳힌다 — 규칙은 `house.md` · 양식은 `templates/` · 방법은 `analysis/methods/`. 굳히기 전에 **범위를 한 줄 되묻는다**. "이번에는" 은 굳히지 않는다. 굳은 파일 머리에 언제부터 · 누가 · 무엇을 보고. 사는 자리는 `CLAUDE.md` 한 줄 + 오케스트레이터의 절 (모든 답변에 걸려야 해서 스킬이 못 받는다)
- **`analysis` 스킬** (ADR-034): 분석 한 건 = `analysis/<날짜>-<slug>/`(`run.py` · 여섯 칸 `run.md`) + **카드 하나**. 관문은 **모형 고르기 한 자리**에만. 같은 물음이 다시 오면 앞 카드를 대고 있는 `run.py` 를 다시 돌린다. **스킬 본체는 고정, 방법은 `analysis/methods/` 데이터** — t 검정·Cpk·DOE 를 미리 스크립트로 만들지 않는다
- **`retro` 스킬** (ADR-036): 사람의 말로 부른다 (cron 없음). 보고 넷 — 되풀이된 것 · 막힌 것 · 굳힐 후보 · 스킬/에이전트 후보(**판별 넷을 통과한 것만**). **항목마다 근거**, 근거 없으면 안 쓴다. 봇은 제안까지, 만드는 것은 사람 + PR
- **`report` 가 `templates/` 를 읽는다**: 이번 첨부 → `templates/` 하나 → **둘 이상이면 사람이 고른다** → 없으면 청한다
- **`journal` 에 `## 되풀이된 말`**: 관찰형이 쌓이는 한 줄. **세지 않는다** — 상태 파일도 카운터도 없다
- 끝 조건: 단위 시험이 트리거와 규격을 보고 `npm test` · `npm run test:server` 실패 0. 대본은 meta 가 사람과 함께 돌려 **과제 폴더의 파일**로 센다 (`VERIFICATION.md` 5.1 의 다섯)

### T5.C 관문 C — 짐 (회사로 옮길 것) — meta 지시
- 회사로 넘어가는 짐을 싼다 — 판정 근거가 된 `../meta/…/runs/` 를 `docs/evidence/` 로 사본, 절대 경로 안내문, 기억 파일을 md 로, `HANDOFF.md` 의 못 박을 두 줄. **커밋·PR 이력은 안 간다**
- 세부는 meta 가 관문 C 를 지시할 때 이 절에 채운다
