...
- 그 자리에 **무엇이 어떻게 들어가는지**를 약속한다: 분석 여섯 칸 · `retro` 스킬(열넷째) · `report` 가 `templates/` 를 읽는 고리 · 굳는 길의 규칙(지시형 · 관찰형)
- 세부는 meta 가 관문 B 를 지시할 때 이 절에 채운다. `VERIFICATION.md` 의 "재는 것 다섯" 도 그때 들어온다

### T5.C 관문 C — 짐 (분석 · DOE) — meta 지시
- 그 길을 타는 **첫 화물**. 결정을 바꾸는 숫자를 내는 것만 스크립트로 만들고, 해석은 만들지 않는다 (판별 넷)
