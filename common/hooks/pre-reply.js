...
// ── 확정 다섯 조건 (ADR-008) ──────────────────────────────
// confirmed_at 글이
//   ① author_type='user'  ② 같은 과제의 방 (ADR-039 — 갈래를 보지 않는다)  ③ 본문이 확정 어휘로 시작
...
function 확정검사(카드id, 과제이름) {
  ...
  // ② 같은 과제의 방 (ADR-039). 방 하나 판이라 갈래를 보지 않는다 — 이관된 옛 <과제>/files 갈래 이름의 방에서 받은 확정도 그 과제의 것이다
  if (!방 || P.roomParts(방.name).과제 !== 과제이름) {
    return `${카드id} 의 확정 글 #${확정번호} 이 ${과제이름} 의 방이 아니다 (${방 ? 방.name : '방 모름'})`;
  }
...
function main() {
  ...
  if (카드표식.test(첫줄)) {
    ...
    // 방을 모르면 조건 ②(같은 과제의 방)를 확인할 길이 없다. fail-closed 다.
